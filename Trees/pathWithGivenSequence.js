class TreeNode {

  constructor(value) {
    this.value = value;
    this.left = null;
    this.right = null;
  }
};

/*
Problem Statement #
Given a binary tree and a number sequence, find if the sequence is present as a root-to-leaf path in the given tree.
*/
// O(n) Time | O(n) Space
const find_path = function (root, sequence) {
  return findSequencePath(root, sequence);
};


const findSequencePath = (root, sequence) => {
  if (root === null) return;
  if (root.value !== sequence[0]) return false;
  sequence.shift();

  if (!root.left && !root.right && !sequence.length) return true;

  return findSequencePath(root.left, sequence) || findSequencePath(root.right, sequence);
}


var root = new TreeNode(1)
root.left = new TreeNode(0)
root.right = new TreeNode(1)
root.left.left = new TreeNode(1)
root.right.left = new TreeNode(6)
root.right.right = new TreeNode(5)

console.log(`Tree has path sequence: ${find_path(root, [1, 0, 7])}`)
console.log(`Tree has path sequence: ${find_path(root, [1, 1, 6])}`)
